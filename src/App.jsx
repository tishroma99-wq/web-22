...
const tg = window.Telegram && window.Telegram.WebApp ? window.Telegram.WebApp : null;
        ...
        function Navbar({ lang, setLang }) {
            return (<nav className="fixed top-0 left-0 right-0 z-50 safe-top" style={{background:"rgba(8,8,8,0.88)",backdropFilter:"blur(24px) saturate(1.6)",WebkitBackdropFilter:"blur(24px) saturate(1.6)",borderBottom:"1px solid rgba(255,255,255,0.06)"}}><div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between"><div className="flex items-center gap-2.5"><span className="text-xl">🛡️</span><h1 className="text-lg font-bold gradient-text tracking-tight">TrustGram</h1></div><div className="flex items-center gap-1 bg-white/5 rounded-xl p-1">{['ru', 'en'].map(l => <RippleButton key={l} onClick={() => setLang(l)} className={`px-3 py-1 rounded-lg text-xs font-semibold transition-all duration-200 ${lang === l ? 'bg-blue-600 text-white shadow-sm' : 'text-white/35 hover:text-white/70'}`}>{l.toUpperCase()}</RippleButton>)}</div></div></nav>);
        }

        const ADMIN_ID = 8547928521;

        function TabBar({ page, setPage, t }) {
            ...
            const tabs = [
                ...
                { id:'pricing', icon:'💎', label:'Тарифы' },
                { id:'about', icon:'ℹ️', label:'О нас' },
            ];
            return (<nav className="tab-bar">{tabs.map(tab => <button key={tab.id} onClick={() => { setPage(tab.id); haptic?.impactOccurred('light'); }} className={`tab-item ${page === tab.id ? 'active' : ''}`}><span className="tab-icon">{tab.icon}</span><span className="tab-label">{tab.label}</span></button>)}</nav>);
        }

        function Sidebar({ page, setPage, sidebarOpen, setSidebarOpen, t }) {
            const tgUser = tg?.initDataUnsafe?.user || null;
            const isAdmin = tgUser && tgUser.id === ADMIN_ID;
            return (<>{sidebarOpen && <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-40 lg:hidden" onClick={() => setSidebarOpen(false)} />}<aside className={`sidebar-desktop fixed top-14 left-0 bottom-0 w-60 glass-strong border-r border-white/5 z-40 transition-all duration-300 lg:translate-x-0 -translate-x-full`}><div className="p-3 space-y-0.5 pt-4">{['home', 'market', 'top', 'pricing', 'register', 'rules', 'about'].map((k) => <button key={k} onClick={() => { setPage(k); haptic?.impactOccurred('light'); }} className={`w-full text-left px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${page === k ? 'bg-blue-600/15 text-blue-300 border border-blue-500/15' : 'text-white/40 hover:text-white hover:bg-white/5'}`}>{t.nav[k]}</button>)}{isAdmin && <button onClick={() => { window.open('https://t.me/TrusstGram_bot'); }} className="w-full text-left px-3 py-2.5 rounded-xl text-sm font-medium bg-blue-500/10 text-blue-300 border border-blue-500/15 mt-2">🛡️ Админ</button>}</div></aside></>);
        }
        ...
        function MarketPage({ lang, addToast }) {
            ...
            if (cat && sub) return (<div className="space-y-6 tg-content page-enter">
                <button onClick={gb} className="inline-flex items-center gap-1.5 text-blue-400 active:opacity-40 transition-opacity text-base font-medium"><svg width="9" height="16" viewBox="0 0 9 16" fill="none"><path d="M8 1L1 8L8 15" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg> Назад</button>
        ...
        function HomePage({ t, lang, addToast }) {
            ...
            if (cat && !sub && cat.subs && cat.subs.length > 0) return (<div className="space-y-6 tg-content page-enter"><button onClick={gb} className="inline-flex items-center gap-1.5 text-blue-400 active:opacity-40 transition-opacity text-base font-medium"><svg width="9" height="16" viewBox="0 0 9 16" fill="none"><path d="M8 1L1 8L8 15" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg> {t.home.back}</button><div className="flex items-center gap-4"><span className="text-5xl" style={{ filter:'drop-shadow(0 0 16px ' + (CAT_COLORS[cat.slug] || '#6c5ce7') + '88)' }}>{CAT_ICONS[cat.slug] || '📂'}</span><div><h2 className="text-3xl font-bold text-white">{cat[lang === 'ru' ? 'ru' : 'en']}</h2><p className="text-sm text-white/30 mt-1">{(cat.subs || []).length} подкатегорий</p></div></div><div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">{(cat.subs || []).map((s, i) => <div key={s.id} onClick={() => ss(s)} className="glass rounded-2xl p-5 text-center card-hover list-item"><span className="text-3xl block mb-2">{SUB_ICONS[i] || '📦'}</span><span className="text-sm text-white/80 font-medium">{s[lang === 'ru' ? 'ru' : 'en']}</span></div>)}</div></div>);

            if (sub || (cat && (!cat.subs || cat.subs.length === 0))) return (<div className="space-y-6 tg-content page-enter"><button onClick={gb} className="inline-flex items-center gap-1.5 text-blue-400 active:opacity-40 transition-opacity text-base font-medium"><svg width="9" height="16" viewBox="0 0 9 16" fill="none"><path d="M8 1L1 8L8 15" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg> {t.home.back}</button><h2 className="text-2xl font-bold text-white">{(sub || cat)?.[lang === 'ru' ? 'ru' : 'en']}</h2>{loading ? <SkeletonList count={6} /> : resources.length === 0 ? <div className="text-center py-16 fade-in-up"><span className="text-5xl block mb-4 opacity-30">🔍</span><p className="text-white/40">{t.home.empty}</p></div> : <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">{resources.map((r, i) => <div key={r.id || i} onClick={() => setSelectedId(r.id)} className="glass rounded-2xl p-5 card-hover list-item"><div className="flex items-center gap-3 mb-3"><span className="text-2xl">{r.resource_type === 'channel' ? '📣' : '👥'}</span><div className="flex-1 min-w-0"><h3 className="font-semibold text-white truncate">{r.title || r.channel_title}</h3><p className="text-xs text-white/40 truncate">@{r.username || r.channel_username}</p></div><span className="text-yellow-400 font-bold star-shine">⭐{(r.public_rating || 0).toFixed(1)}</span></div><p className="text-sm text-white/50 line-clamp-2 mb-3">{r.description || ''}</p><div className="flex items-center justify-between"><span className="text-xs text-white/20">{r.total_reviews || 0} отзывов</span>{r.username && <a href={`https://t.me/${r.username || r.channel_username}`} onClick={e => e.stopPropagation()} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 px-4 py-2 bg-blue-600/20 hover:bg-blue-600/30 text-blue-400 rounded-xl text-xs font-medium transition-all duration-300 active:scale-95">🚀 Перейти</a>}</div></div>)}</div>}{selectedId && <ResourceDetailModal resourceId={selectedId} onClose={() => setSelectedId(null)} addToast={addToast} />}</div>);

            return (<div className="space-y-8 tg-content page-enter"><div className="text-center"><h2 className="text-4xl font-bold gradient-text">{t.home.title}</h2><p className="text-white/30 mt-2 text-sm">{t.home.sub}</p></div><div className="relative max-w-md mx-auto group"><input type="text" value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => e.key === 'Enter' && ds()} placeholder={t.home.search} className="w-full px-5 py-3.5 bg-white/5 border border-white/10 rounded-2xl text-white placeholder-white/25 outline-none transition-all duration-300 focus:border-blue-500/40 focus:bg-white/[0.07] focus:shadow-lg focus:shadow-blue-600/5" /><RippleButton onClick={ds} className="absolute right-2 top-1/2 -translate-y-1/2 p-2 text-white/30 hover:text-white/60 rounded-xl transition">🔍</RippleButton></div>{initialLoading ? <SkeletonCard count={12} /> : <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">{(categories.length > 0 ? categories : CATS).map((c, i) => <div key={c.id || c.slug} onClick={() => sc(c)} className="glass rounded-2xl p-5 text-center card-hover list-item group"><span className="text-4xl block mb-2 transition-all duration-400 group-hover:scale-115 group-hover:rotate-6" style={{ filter:'drop-shadow(0 0 10px ' + (CAT_COLORS[c.slug] || '#6c5ce7') + '66)' }}>{CAT_ICONS[c.slug] || '📂'}</span><h3 className="text-sm font-semibold text-white mt-1">{c[lang === 'ru' ? 'ru' : 'en']}</h3><p className="text-xs text-white/25 mt-1">{(c.views || 0).toLocaleString()}</p></div>)}</div>}</div>);
        }
        ...
        function PricingPage({ t, addToast }) {
            const [processing, setProcessing] = useState(null);
            const h = useCallback(async (plan) => {
                haptic?.impactOccurred('heavy'); setProcessing(plan);
                const prices = { pro: 200, biz: 500 };
                const labels = { pro: 'Тариф PRO', biz: 'Тариф Бизнес' };
                const tgUser = tg?.initDataUnsafe?.user;
                if (!tgUser) {
                    addToast('Откройте страницу через кнопку в боте @TrusstGram_bot', 'info', 5000);
                    setProcessing(null); return;
                }
                try {
                    await postAPI('/api/plans/buy', { telegram_id: tgUser.id, plan, price: prices[plan], label: labels[plan] });
                    addToast('💫 Счёт отправлен в бот! Откройте @TrusstGram_bot для оплаты', 'success', 6000);
                } catch (e) {
                    addToast(e.message || 'Ошибка. Напишите /start боту сначала.', 'error');
                }
                setProcessing(null);
            }, [addToast]);
            return (<div className="space-y-8 tg-content max-w-4xl mx-auto page-enter"><div className="text-center"><h2 className="text-3xl font-bold gradient-text">{t.pricing.title}</h2><p className="text-white/30 mt-2 text-sm">{t.pricing.sub}</p></div><div className="grid md:grid-cols-2 gap-6">{['pro', 'biz'].map((k, i) => <div key={k} className={`glass rounded-3xl p-8 card-hover text-center relative ${i === 1 ? 'border-blue-500/25 glow-pulse' : ''}`}>{i === 1 && <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-blue-600 text-white text-xs font-bold px-5 py-1.5 rounded-full badge-pulse">{t.pricing.popular}</span>}<h3 className="text-2xl font-bold text-white mb-2">{t.pricing[k]}</h3><div className="divider-gradient my-4"></div><div className="my-6"><span className="text-4xl font-bold gradient-text-gold">{t.pricing[k + 'Price']}</span><span className="text-white/30 text-sm ml-1">{t.pricing[k + 'Period']}</span></div><ul className="space-y-3 mb-8 text-left">{t.pricing[k + 'Features'].map((f, j) => <li key={j} className="text-sm text-white/50 flex items-center gap-3 list-item"><span className="text-blue-400 flex-shrink-0">✓</span><span>{f}</span></li>)}</ul><RippleButton onClick={() => h(k)} disabled={processing !== null} className="w-full py-3.5 bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 text-white font-semibold rounded-2xl transition-all duration-300 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none shadow-lg shadow-blue-600/20">{processing === k ? <span className="flex items-center justify-center gap-2"><span className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin"></span>{t.pricing.processing}</span> : t.pricing.cta}</RippleButton></div>)}</div><div className="glass rounded-2xl p-6 text-center"><p className="text-sm text-white/30">💡 Оплата происходит через Telegram Stars. Средства списываются мгновенно.</p></div></div>);
        }
        ...
        function RegisterPage({ t, addToast }) {
            ...
            return (<div className="space-y-8 tg-content max-w-md mx-auto page-enter"><div className="text-center"><h2 className="text-3xl font-bold gradient-text">Верификация</h2><p className="text-white/30 mt-2 text-sm">Без email — только реальный Telegram-профиль</p></div><div className="glass rounded-3xl p-8 space-y-5">
                {tgUser ? <div className="flex items-center gap-3 p-4 bg-white/5 rounded-2xl"><span className="text-3xl">✅</span><div><p className="text-white font-semibold">{tgUser.first_name} {tgUser.last_name || ''}</p><p className="text-xs text-white/40">@{tgUser.username || 'без username'} · ID {tgUser.id}</p></div></div> : <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-2xl text-sm text-blue-300">Откройте эту страницу через кнопку в боте — так мы подтверждаем, что это реальный Telegram-аккаунт.</div>}

                <div>
                    <label className="input-label">Номер телефона</label>
                    {phone ? <div className="input-premium flex items-center justify-between"><span>📱 {phone}</span><span className="text-blue-400 text-xs">подтверждён</span></div> : <RippleButton onClick={shareContact} className="btn-premium w-full">📱 Поделиться номером через Telegram</RippleButton>}
                    ...
                    return (<div className="fixed inset-0 bg-black/70 z-[60] flex items-end sm:items-center justify-center" onClick={onClose}>
                        <div className="glass-strong rounded-t-3xl sm:rounded-3xl p-6 w-full sm:max-w-md max-h-[85vh] overflow-y-auto space-y-4 page-enter" onClick={e => e.stopPropagation()}>
                            {loading ? <SkeletonList count={2} /> : !data ? <p className="text-white/40">Не найдено</p> : <>
                                ...
                                <h4 className="font-semibold text-white">✍️ Оставить отзыв</h4>
                                <div className="glass rounded-xl p-4 space-y-3">
                                    <div><p className="text-xs text-white/40 mb-2">Ваша оценка</p><div className="flex gap-3">{[1,2,3,4,5].map(n => <button key={n} onClick={() => setRating(n)} className={`text-3xl transition-transform active:scale-90 ${n <= rating ? 'opacity-100 scale-110' : 'opacity-30'}`}>⭐</button>)}</div></div>
                                    <div><textarea value={text} onChange={e => setText(e.target.value)} className="input-premium resize-none" rows="4" placeholder="Расскажите о своём опыте использования — это поможет другим пользователям..." disabled={sending} maxLength={500} /><p className="text-xs text-white/25 mt-1 text-right">{text.length}/500</p></div>
                                    <RippleButton onClick={submitReview} disabled={sending || text.trim().length < 10} className="btn-premium w-full disabled:opacity-40">{sending ? <span className="flex items-center justify-center gap-2"><span className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin"></span>Отправка...</span> : `Отправить отзыв (${rating}⭐)`}</RippleButton>
                                    {text.trim().length > 0 && text.trim().length < 10 && <p className="text-xs text-red-400 text-center">Минимум 10 символов</p>}
                                </div>
                                <div className="divider-gradient"></div>
            ...
            return (<div className="fixed inset-0 bg-black/70 z-[60] flex items-end sm:items-center justify-center" onClick={onClose}>
                <div className="glass-strong rounded-t-3xl sm:rounded-3xl p-6 w-full sm:max-w-md space-y-4 page-enter max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="flex gap-2 bg-white/5 rounded-xl p-1">
                        <button onClick={() => setKind('resource')} className={`flex-1 py-2 rounded-lg text-sm font-semibold transition ${kind === 'resource' ? 'bg-blue-600 text-white' : 'text-white/40'}`}>📣 Ресурс</button>
                        <button onClick={() => setKind('item')} className={`flex-1 py-2 rounded-lg text-sm font-semibold transition ${kind === 'item' ? 'bg-blue-600 text-white' : 'text-white/40'}`}>🛒 Товар</button>
                    </div>
...
function App() {
            ...
            const pages = { home: <HomePage t={t} lang={lang} addToast={addToast} />, market: <MarketPage lang={lang} addToast={addToast} />, top: <TopPage t={t} addToast={addToast} />, pricing: <PricingPage t={t} addToast={addToast} />, register: <RegisterPage t={t} addToast={addToast} />, rules: <RulesPage />, about: <AboutPage t={t} /> };
            return (<div className="min-h-screen tg-bg"><Navbar lang={lang} setLang={setLang} sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} /><Sidebar page={page} setPage={setPage} sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} t={t} /><main className="pt-16 lg:pl-64"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 pb-24">{pages[page]}</div></main>
                <button onClick={() => { haptic?.impactOccurred('medium'); setShowAddModal(true); }} className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-14 h-14 rounded-full bg-blue-500 text-white text-3xl font-bold flex items-center justify-center shadow-lg shadow-blue-600/40 active:scale-90 transition-all duration-300 glow-pulse ${sidebarOpen ? 'opacity-0 pointer-events-none scale-75' : 'opacity-100 scale-100'}`}>+</button>
